...
const API_KEY = 'zaneru-official'; // <-- DONT CHANGE
...
const logger = winston.createLogger({
    ...
    transports: [
        ...
        new winston.transports.File({ filename: 'Zaneru.log' })
    ]
});

async function getApiLink(content, type) {
    const baseUrl = 'https://zaneru-official.vercel.app/api/bypass';
    const endpoints = {
        fluxus: 'fluxus',
        relzhub: 'relzhub',
        delta: 'delta',
    ...
    return endpoints[type] ? `${baseUrl}/${endpoints[type]}?link=${content}&api_key=${API_KEY}` : null;
}
